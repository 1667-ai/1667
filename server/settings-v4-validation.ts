import {
  ...
  type GenerationProfileV4,
  type ModelDefinitionV4,
...
const PROFILE = closedShape(
  ["name", "modelId", "temperature", "maxOutputTokens", "effort", "cachePolicy", "thinkingMode"],
...
/** Parse one schema-4 document without projecting its profile controls. */
export function validateSettingsDocumentV4(
  value: unknown,
  options: SettingsValidationOptions = {}
): SettingsDocumentV4 {
  const root = closedRecord(value, "settings document", DOCUMENT);
  literal(root.schemaVersion, 4, "settings document.schemaVersion");
  ...
  const profiles = parseProfilesV4(root.profiles, models, connections);
  const routing = parseRoutingV4(root.routing, profiles);
...
function parseProfilesV4(
  value: unknown,
  models: Readonly<Record<string, ModelDefinitionV4>>,
  connections: Readonly<Record<string, ModelConnectionV2>>
): Record<string, GenerationProfileV4> {
  const record = settingsMap(value, "settings document.profiles");
  const result: Record<string, GenerationProfileV4> = {};
  for (const [id, raw] of Object.entries(record)) {
    ...
    if (model === undefined) throw new SettingsFormatError(`profile ${id}.modelId does not resolve`);
    const connection = connections[model.connectionId];
    if (connection === undefined) throw new SettingsFormatError(`model ${modelId}.connectionId does not resolve`);
    ...
    const effort = oneOf(profile.effort, GENERATION_EFFORT_V4_VALUES, `profile ${id}.effort`);
    const thinkingMode = oneOf(profile.thinkingMode, THINKING_MODE_V4_VALUES, `profile ${id}.thinkingMode`);
    ...
    const parsed: GenerationProfileV4 = {
