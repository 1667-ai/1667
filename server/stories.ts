 ...
 * snapshot still has pinned, and every Image Object a live Draft Lease
 * references but no manifest node names yet (`runCleanup` below passes that
 * story's lease-sourced ids as `liveImageIds`). This is the one place "an
 * object is live when the manifest names it or a lease does" is decided,
 * `StoryObjectStore.sweep` itself is kind-generic and knows nothing about
 * leases. `pinned` is optional because a story with no active provider
 * snapshot has nothing to look up; treating that the same as an empty pin
 * map keeps this the only place the lease union has to happen. */
function unionLiveWithPins(
  live: LiveStoryObjectIds,
  pinned: ProviderSnapshotPins | undefined,
  liveImageIds: readonly ObjectHash[] = []
): LiveStoryObjectIds {
  const leaves = {} as Record<LeafObjectKind, readonly ObjectHash[]>;
  for (const kind of LEAF_OBJECT_KINDS) {
    const pinnedIds = pinned === undefined ? [] : [...pinned[kind].keys()];
    const leaseIds = kind === "images" ? liveImageIds : [];
    leaves[kind] = [...new Set([...live.leaves[kind], ...pinnedIds, ...leaseIds])];
  }
  return {
    revisions: [...new Set([...live.revisions, ...(pinned === undefined ? [] : pinned.revisions.keys())])],
    leaves
  };
}
