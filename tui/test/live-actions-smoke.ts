...
let summaryDeltas = 0;
const summaryNode = await api.createSummaryTake(created.id, { nodeId: withFact.path.at(-1)!.id }, () => { summaryDeltas += 1; }, summaryAbort.signal);
if (summaryNode === null || summaryDeltas < 1) throw new Error("summary take did not stream and land");
