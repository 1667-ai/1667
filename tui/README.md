...
HTTP server mode requires Linux. The machine tier must be outside the project. The data filesystem must support durable Linux file handles. The client proves the server on each exact TCP connection before it sends a capability or request body. It then preflights product and API compatibility before each operation. It sends the negotiated client protocol and the random server-instance ID with the operation. Protocol v9 adds the data-directory ID and data-directory claim ID to health metadata. The server sends these IDs only after it validates a client capability. Git can track the data-directory ID. The client uses the data-directory ID when it retries an unfinished create or import after a port change. The client uses the data-directory claim ID to reject a copied directory as a replacement listener. Linux HTTP mode requires Linux kernel 6.8 or later. On Linux Btrfs, Bun, Node, and the standalone executable query the kernel through runtime adapters. They do not run the `btrfs` command. On Linux, a filesystem remount stops the retry. This rule prevents a cloned filesystem from taking over the retry. A different project cannot use that unfinished operation. Thus, older independent servers are rejected before an inline `e` save can reach them. The loopback server binds each mutation to that exact instance before dispatch. This closes the restart gap between health negotiation and a story mutation. If preflight first reports recovered state, the triggering mutation is not sent. The client closes an interrupted provider record and reloads the authoritative story and settings. If recovery leaves an empty library, only the replacement-story create is permitted. Provider-backed generation requests use the 30-minute generation-class deadline. Ordinary unary HTTP actions retain a 15-second bound.
