...
import { legacyGenerationReasoningV5 } from "../shared/settings-v5-reasoning.js";
import {
  ASIDE_EXPORT_OMISSION_NOTICE,
  emptyAsideDocument,
  appendSideNote,
  type AsideDocument as DemoAsideDocument
} from "../shared/aside-core.js";
import {
  buildSearchCorpus,
  createSearchScan,
...
import {
  createFactState as createDemoFactState,
  deleteFactState as deleteDemoFactState,
  patchFactState as patchDemoFactState
} from "../shared/story-fact-mutations.js";
import { deleteSubtree as deleteDemoSubtree } from "../shared/story-node-delete.js";
import {
...
export function createDemoController(dense = false): DemoController {
  ...
  const deleteDemoNode = (nodeId: string, expectedSubtreeCount: number): StoryPayload => {
    ...
    deleteDemoSubtree(story, nodeId, expectedSubtreeCount);
  ...
  return {
    ...
    createFactState(id, input) {
      ...
      createDemoFactState(story, id, input, stateId);
    ...
    patchFactState(id, stateId, input) {
      ...
      patchDemoFactState(story, id, stateId, input);
      normalizeDemoFactClocks(story, id);
      return payloadFrom(story);
    },
    deleteFactState(id, stateId) {
      ...
      deleteDemoFactState(story, id, stateId);
      normalizeDemoFactClocks(story, id);
      return payloadFrom(story);
    },
...
const { effort: _demoEffort, ...DEMO_PROFILE_V5_BASE } = DEMO_SETTINGS_DOCUMENT_V2.profiles.default!;
export const DEMO_SETTINGS_DOCUMENT: SettingsDocumentV5 = {
  schemaVersion: 5,
  connections: DEMO_SETTINGS_DOCUMENT_V2.connections,
  models: Object.fromEntries(
    Object.entries(DEMO_SETTINGS_DOCUMENT_V2.models).map(([id, model]) => [
      id,
      { ...model, capabilities: { ...model.capabilities, imageInput: "unsupported" as const } }
    ])
  ),
  profiles: {
    default: {
      ...DEMO_PROFILE_V5_BASE,
      generationReasoning: legacyGenerationReasoningV5("default")
    }
  },
  routing: DEMO_SETTINGS_DOCUMENT_V2.routing,
  writing: writingPromptSettingsFromAuthorBrief(DEMO_SETTINGS_DOCUMENT_V2.writing.defaultAuthorBrief)
};
...
export function demoStoryApi(demo: DemoController): StoryApi {
  ...
  const asideDocuments = new Map<string, DemoAsideDocument>();
