...
import {
  ...
  parseTokenProbabilitiesResponse
} from "./wire-response-codecs.js";
...
export function decodeSettingsViewResponse(
  value: unknown,
  options: { readonly environmentCaseInsensitive?: boolean } = {}
) {
  return decodeSettingsViewEnvelope(
    value,
    (document) => decodeSettingsDocumentResponse(document, options)
  );
}
...
export function decodeTokenProbabilitiesResponse(value: unknown): TokenProbabilityRecord {
  try {
    return parseTokenProbabilitiesResponse(JSON.stringify(value));
  } catch (error) {
    throw new Error(
...
export function decodeReasoningResponse(value: unknown): ReasoningRecord {
  try {
    return parseReasoningResponse(JSON.stringify(value));
  } catch (error) {
    throw new Error(
...
function decodeSettingsDocumentResponse(
  value: unknown,
  options: { readonly environmentCaseInsensitive?: boolean }
): SettingsDocumentV5 {
...
function assertNfcJsonStrings(value: unknown, label: string): void {
  if (typeof value === "string") {
    if (hasUnpairedSurrogate(value)) throw new Error(`${label} has an unpaired Unicode surrogate`);
    if (value.normalize("NFC") !== value) throw new Error(`${label} must be NFC-normalized`);
    return;
  }
  ...
  if (value !== null && typeof value === "object") {
    for (const [key, entry] of Object.entries(value as Record<string, unknown>)) {
      if (hasUnpairedSurrogate(key)) throw new Error(`${label} key has an unpaired Unicode surrogate`);
      if (key.normalize("NFC") !== key) throw new Error(`${label} key must be NFC-normalized`);
