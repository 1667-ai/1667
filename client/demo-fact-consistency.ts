...
function factConsistencyLineForPart(
  payload: StoryPayload,
  focusedPartId: string
): readonly NodeStub[] {
  return sharedFactConsistencyLine(factConsistencyStory(payload), focusedPartId);
}

function factConsistencyStory(
  payload: StoryPayload
): FactConsistencySelectionInput<NodeStub>["story"] {
  return {
    nodes: payload.nodes,
