...
import { downloadPlatformPackage } from "../../host/launcher/upgrade-download.js";
import { executeUpgradeCli } from "../src/upgrade-cli.js";
import { createUpgradeProgressRenderer } from "../../host/launcher/upgrade-progress.js";
import {
...
test("download progress is bounded when transport chunks are tiny", async () => {
  ...
  try {
    ...
    const progress: import("../../host/launcher/upgrade-download.js").PackageDownloadProgress[] = [];
    await downloadPlatformPackage({
      tarballUrl: canonicalNpmTarballUrl(PACKAGE, "1.0.0"),
      packageName: PACKAGE,
      version: "1.0.0",
...
test("stalled package download headers are a retryable network failure", async () => {
  ...
  try {
    ...
    const { UpgradeFailure } = await import("../../host/launcher/upgrade-contract.js");
    expect(failed instanceof UpgradeFailure).toBe(true);
    const failure = failed as import("../../host/launcher/upgrade-contract.js").UpgradeFailure;
...
test("canonical package fixtures reject missing required entries", async () => {
  ...
  try {
    ...
    const { materializeCandidate } = await import("../../host/launcher/upgrade-candidate.js");
